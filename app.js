...
// [13] Load data auto-generate dari berbagai tabel - DIPERBAIKI
async function loadAutoGenerateData(tanggal) {
    ...
    try {
        ...
        // 3. Komisi, Tips QRIS dari tabel komisi - UOP DIPISAH
        try {
        ...
        // 4. UOP HANYA untuk barberman - DIPERBAIKI
try {
    // ✅ Ambil data karyawan barberman di outlet ini
    const { data: barbermanData, error: barbermanError } = await supabase
        .from('karyawan')
        .select('nama_karyawan')
        .eq('outlet', outlet)
        .eq('role', 'barberman');
    
    if (!barbermanError && barbermanData && barbermanData.length > 0) {
        // ✅ Ambil UOP hanya untuk barberman
        const barbermanNames = barbermanData.map(b => b.nama_karyawan);
        
        const { data: uopData, error: uopError } = await supabase
            .from('komisi')
            .select('uop, serve_by')  // ✅ UBAH: nama_karyawan → serve_by
            .eq('outlet', outlet)
            .eq('tanggal', tanggal)
            .in('serve_by', barbermanNames);  // ✅ UBAH: nama_karyawan → serve_by
        
        if (!uopError && uopData && uopData.length > 0) {
            data.uop = uopData.reduce((sum, item) => 
                sum + (parseFloat(item.uop) || 0), 0);
            
            console.log('✅ UOP untuk barberman ditemukan:', {
                uop: data.uop,
                barberman: barbermanNames,
                records: uopData.length,
                detail: uopData // ✅ Tambah detail untuk debug
            });
        } else {
            console.log('ℹ️ Tidak ada UOP untuk barberman di tanggal ini');
            if (uopError) console.log('Error detail:', uopError);
        }
    } else {
        console.log('ℹ️ Tidak ada barberman di outlet ini');
    }
} catch (e) {
