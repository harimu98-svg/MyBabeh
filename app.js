...
function canPerformSetoran() {
    try {
        ...
        // 5. Cek ada saldo KAS atau tidak
        const totalSaldo = kasState.currentKasData.reduce((sum, row) => {
            return sum + (parseInt(row.saldo) || 0);
        }, 0);
        
        if (totalSaldo <= 0) {
            console.log('❌ Tidak ada saldo KAS untuk setoran:', totalSaldo);
            return false;
        }
...
function updateSetorButton() {
    try {
        ...
        console.log('🔄 Update tombol SETOR:', { canSetor });
        ...
        if (!canSetor) {
            ...
            if (!isSetoranValid()) {
                setorBtn.innerHTML = '<i class="fas fa-ban"></i> PILIH PERIODE';
