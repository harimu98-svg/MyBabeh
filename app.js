...
async function loadAutoGenerateData(tanggal) {
    ...
    try {
        ...
        if (transaksiData && transaksiData.length > 0) {
            data.omset_cash = transaksiData.reduce((sum, item) => sum + (parseInt(item.total_amount) || 0), 0);
        }
        ...
        if (komisiData && komisiData.length > 0) {
            data.komisi = komisiData.reduce((sum, item) => sum + (parseInt(item.komisi) || 0), 0);
            data.uop = komisiData.reduce((sum, item) => sum + (parseInt(item.uop) || 0), 0);
            data.tips_qris = komisiData.reduce((sum, item) => sum + (parseInt(item.tips_qris) || 0), 0);
        }
